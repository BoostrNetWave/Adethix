...
import CampaignReportTable from "./CampaignReportTable.jsx";
import { Link as RouterLink, useParams } from "react-router-dom";

function CampaignReport({ campaign, ads, reportInfo, totalReportInfo }) {
  ...
  useDocumentTitle(
    campaign?.name
      ...
      : "Campaign Report: Advertisement Platform"
  );
    ...
    if (endDate !== "" && startDate !== "") {
      ...
      try {
        const response = await axiosInstance.get(
          `/api/advertiser/campaign/${campaignId}/report/range`,
          { params }
        );
  ...
  return (
    <>
      ...
      {updatedReportInfo && updatedReportInfo?.length > 0 ? (
        <Grid item xs={12} md={7} mt={2} lg={8}>
          ...
          <MainCard sx={{ mt: 2 }} content={false}>
            <CampaignReportTable data={updatedReportInfo} />
          </MainCard>
        </Grid>
      ) : (
      ...
      {/* show link to see advertisemnet wise report */}
      <Box>
        <Typography variant="h6" mb={0.5} mt={2} className={styles.reportTitle}>
